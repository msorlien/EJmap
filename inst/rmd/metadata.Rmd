...
scale. Please review the [**technical documentation**](https://narragansett-bay-estuary-program-nbep.hub.arcgis.com/documents/396d763941a34e57a2e6eebf05f82b8b/explore)
...
df_dataSource <- params$df_metrics %>%
  ...
  mutate(DATA_SOURCE = case_when(
    ...
    DATA_SOURCE == 'CDC' ~ 'Centers for Disease Control',
    DATA_SOURCE == 'DOT' ~ 'Department of Transportation',
    DATA_SOURCE == 'EPA' ~ 'Environmental Protection Agency',
    DATA_SOURCE == 'NLCD' ~ 'National Landcover Database',
    DATA_SOURCE == 'NOAA' ~ 'National Oceanic and Atmospheric Administration',
    DATA_SOURCE == 'NLCD, USFS' ~ 
      'National Landcover Database, US Forestry Service',
    TRUE ~ DATA_SOURCE)) %>%
  # Add new column that concatenates data source, source year
  mutate(DATA_SUMMARY = paste0(DATA_SOURCE, ' (', SOURCE_YEAR, ')'))

list_dataSource <- paste(df_dataSource$DATA_SUMMARY, collapse = '; ')
